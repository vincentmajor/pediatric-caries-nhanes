...
df.train = df.full %>% 
  ...
  ungroup() %>% 
  select(-SEQN, -counter, -sound, -unrestored, -sealant) %>% 
  mutate(unsound = as.integer(unsound > 0))
...
library(rpart)
rpart(unsound ~ ., data = df.train)

install.packages("randomForestSRC")
library(randomForestSRC)
model.rfsrc = rfsrc(unsound ~ ., 
                    data = df.train,
                    na.action = "na.impute")

install.packages("infotheo")
library(infotheo)
a = mutinformation(discretize(select(df.train, income_ratio_over_poverty), disc = "equalfreq"), select(df.train, unsound))


#### Information value ----
if(!require(Information)){install.packages("Information");library(Information)}
IV <- create_infotables(data=df.train, y="unsound", ncore=2)

# Show the first records of the IV summary table
print(IV$Summary, row.names=FALSE)
a = data.frame(prop_missing = sapply(df.train, function(x) mean(is.na(x))))
a$Variable = rownames(a)
IV.summary = left_join(IV$Summary, a)
...
#breastfeeding
df.breastfeeding = df.full %>% select(unsound_flag, diet_breastfed_flag:diet_other_started) %>%
...
rpart(factor(unsound_flag) ~ ., data = df.breastfeeding)
missmap(df.breastfeeding)
df.breastfeeding = na.omit(df.breastfeeding)
model.breastfeeding.glm = glm(unsound_flag ~ ., data = select(df.breastfeeding, -diet_formula_ended), family = binomial())
summary(model.breastfeeding.glm)
car::vif(model.breastfeeding.glm)

## smoking status, exhaled_ntr
df.smoking = na.omit(df.full %>% select(unsound_flag, cotinine_bld, nnal_urn, exhaled_nitric_oxide, household_smokers_num, household_cigarettes))
rpart(unsound_flag ~ ., data = df.smoking)
model.smoking.glm = glm(unsound_flag ~ ., data = df.smoking, family = binomial())
...
df.demo = df.full %>% select(unsound_flag, income_ratio_over_poverty:ethnicity)
missmap(df.demo)
df.demo = na.omit(df.demo)
model.demo.glm = glm(unsound_flag ~ ., data = df.demo, family = binomial())
...
library(mgcv)
model.demo.gam = mgcv::gam(unsound_flag ~ s(age, bs = 'ps') + relevel(factor(df.demo$ethnicity), 'white'), data = df.demo, family = binomial())
summary(model.demo.gam)
plot(model.demo.gam)

## plot the outcome over 1 year age bind


## cardio
df.cardio = df.full %>% select(unsound_flag, weight_kg:diabp)
missmap(df.cardio)
